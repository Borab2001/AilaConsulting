...
const HeaderButton = () => {
    ...
    return (
        ...
        <>
            <motion.div 
                className="fixed z-50 top-4 right-4 w-16 h-16 pointer-events-auto bg-bento border border-border text-title p-4 rounded-2xl flex flex-col items-center justify-center transition-all duration-500 ease-in-out cursor-pointer"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 0.5, delay: 0.6, ease: "easeInOut" }}
                onClick={() => setIsActive(!isActive)}
