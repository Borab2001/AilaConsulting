...
const MobileHeader = () => {
    ...
    return (
        <div className={`inset-0 fixed z-10 h-screen w-full transition-all duration-500 ${isActive ? "bg-black/10 backdrop-blur-sm" : "bg-transparent backdrop-blur-none"}`}>

            <motion.header 
                className={`w-header absolute top-4 left-4 ${isActive ? "h-auto bg-white" : "h-auto bg-gray-50"} p-4 rounded-2xl flex flex-col md:hidden transition-colors duration-500 ease-in-out`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
